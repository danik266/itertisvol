...
export default function QuizPage() {
  ...
  const next = async () => {
    if (step < quizQuestions.length - 1) {
      ...
      // Calculate scores
      const scores: ScoreMap = { eco: 0, social: 0, animal: 0, event: 0, edu: 0, crisis: 0, patriotic: 0, medical: 0 };
  ...
  if (result) {
    ...
    const descMap: Record<string, { ru: string; kz: string }> = {
      ...
      patriotic: {
        ru: 'Ты настоящий патриот! Патриотическое волонтёрство и участие в важных государственных инициативах — это твоё!',
        kz: 'Сен нағыз патриотсың! Патриоттық волонтерлік және маңызды мемлекеттік бастамаларға қатысу — сенікі!',
      },
      crisis: {
        ru: 'Ты не боишься трудностей и готов помогать в чрезвычайных ситуациях. Волонтёрство при ЧС ждёт тебя!',
        kz: 'Сен қиындықтардан қорықпайсың және төтенше жағдайларда көмектесуге дайынсың. ТЖ волонтерлігі сені күтеді!',
      },
      medical: {
        ru: 'Медицина и здоровье людей для тебя на первом месте. Медицинское волонтёрство — твоё идеальное направление!',
        kz: 'Медицина және адамдардың денсаулығы сен үшін бірінші орында. Медициналық волонтерлік — сенің мінсіз бағытың!',
      },
      edu: {
        ru: 'Наука и тяга к знаниям — твои сильные стороны. Обучай, просвещай и развивай общество в сфере науки и просвещения!',
        kz: 'Ғылым және білімге құштарлық — сенің мықты тұстарың. Оқыт, ағарт және қоғамды дамыт!',
      },
      event: {
        ru: 'Искусство и креатив всегда с тобой. Рекомендуем попробовать себя в Творческом волонтёрстве!',
        kz: 'Өнер және креатив әрқашан сенімен бірге. Шығармашылық волонтерлікте өзіңді сынап көруді ұсынамыз!',
      },
    };
